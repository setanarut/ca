...
func main() {
	...
	otomat := ca.NewAutomaton(1280, 720, 33, ca.DunesRule(32))
	...
	// Saniyede bir güncelleme yapan goroutine
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				fmt.Printf("\rİşlenen kare sayısı: %d", frames)
			case <-done:
				return
			}
		}
	...
	fmt.Println("\nSimulation time taken:", time.Since(t).Seconds(), "seconds")
